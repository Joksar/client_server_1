...
from errors import ReqFieldMissongError, ServerError
from socket import socket, AF_INET, SOCK_STREAM
from common.variables import DEFAULT_PORT, DEFAULT_IP_ADRESS, \
    MAX_CONNECTIONS, MAX_PACKAGE_LENGTH, ENCODING, \
    ACTION, TIME, USER, ACCOUNT_NAME, PRESENCE, \
    RESPONSE, ERROR, RESPONDEFAULT_IP_ADRESS, MESSAGE, SENDER, MESSAGE_TEXT
...
def create_arg_parser():
    ...
    if client_mode not in ('listen', 'send'):
        CLIENT_LOGGER.critical(f'Указан недопустимый режим работы {client_mode}, '
                               f'допустимые режим: listen, send.')
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        print(f'Установлено соединение с сервером. Принят ответ от сервера {answer}')
        ...
        # Если соединение с сервером установлено корректно, начинаем обмен,
        # согласно требуемому режиму.
        if client_mode == 'send':
            try:
